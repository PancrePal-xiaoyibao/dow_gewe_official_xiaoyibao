...
class ChatGPTBot(Bot, OpenAIImage, OpenAIVision):
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            triggers = self.load_image_recognition_triggers()
            if is_group:
                # 群聊：只有触发词+图片才分析
                if any(trigger in query for trigger in triggers):
                    ...
                    if image_url:
                        prompt = query  # 也可以用默认prompt
                        return self.vision_recognition(image_url, prompt)
            logger.info("[CHATGPT] query={}".format(query))

            if not is_group:
                # 单聊：内容中包含触发词和图片链接就自动分析
                for trigger in triggers:
                    if trigger in query and "http" in query:
                        url_start = query.find("http")
                        image_url = query[url_start:].strip()
                        prompt = query[:url_start].strip()
                        return self.vision_recognition(image_url, prompt)
            ...
            if not is_group:
                ...
                #     replies.append(Reply(type=ReplyType.IMAGE, content=local_path))
                # elif file_url:
                #     replies.append(Reply(type=ReplyType.IMAGE_URL, content=file_url))
                # # 目前只返回分析文本，如需闭环返回图片可取消注释
                if file_url:
                    prompt = "请分析这张图片"
                    replies.append(self.vision_recognition(file_url, prompt))
                else:
                    replies.append(Reply(type=ReplyType.ERROR, content="图片未找到"))
                # 你需要让下游支持批量发送（如循环 send），否则只返回第一个
                return replies[0]  # 或 return replies
            else:
                # 群聊：只有触发词才分析，只返回文本
                last_text = ""
                if "last_text" in context:
                    last_text = context["last_text"]
                if any(trigger in last_text for trigger in triggers):
                    prompt = last_text
                    if file_url:
                        return self.vision_recognition(file_url, prompt)
                    else:
                        return Reply(type=ReplyType.ERROR, content="图片未找到")
                else:
                    return Reply(type=ReplyType.INFO, content="如需分析图片，请在图片前输入分析指令。")
    ...
    def load_image_recognition_triggers(self):
        config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'config.json')
        with open(config_path, 'r') as f:
            config = json.load(f)
        return config.get('image_recognition_triggers', ["图片分析", "分析图片", "报告分析", "分析报告", "请分析"])

    def vision_recognition(self, image_url, prompt="请分析图片"):  # 可根据需要调整prompt
        messages = [
            {"role": "user", "content": [
        ...
        try:
            ...
            logger.error(f"[CHATGPT] vision_recognition error: {e}")
